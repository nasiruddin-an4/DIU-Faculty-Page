...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const filtered = departments.filter((dept) => {
      const matchesFaculty = selectedFaculty ? dept.faculty.id === selectedFaculty : true
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-3/4 lg:w-4/5">
            <h2 className="section-title">
              {selectedFaculty 
                ? `Departments in ${faculties.find(f => f.id === selectedFaculty).name}` 
                : "All Departments"
              }
            </h2>
            ...
            {filteredDepartments.length > 0 ? (
              ...
              <div className="bg-neutral-100 rounded-lg p-8 text-center">
                <p className="text-lg text-neutral-600">No departments found matching your criteria.</p>
                <button 
                  onClick={() => {
                    setSelectedFaculty('')
                    setSearchTerm('')
