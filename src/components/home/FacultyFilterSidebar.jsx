...
const FacultyFilterSidebar = ({ faculties, selectedFaculty, onFacultyChange }) => {
        ...
        {isExpanded && (
          ...
          >
            <div className="flex items-center group">
              ...
              >
                <div className="w-4 h-4 rounded-full border-2 border-gray-300 peer-checked:border-primary-600 mr-3
                              flex items-center justify-center">
                  <div className={`w-2 h-2 rounded-full bg-primary-600 ${selectedFaculty === "" ? 'opacity-100' : 'opacity-0'}`} />
                </div>
                All Faculties
              </label>
            </div>
            ...
            {faculties.map((faculty) => (
              <div key={faculty.id} className="flex items-center group">
                ...
                >
                  <div className="w-4 h-4 rounded-full border-2 border-gray-300 peer-checked:border-primary-600 mr-3
                                flex items-center justify-center">
                    <div className={`w-2 h-2 rounded-full bg-primary-600 ${selectedFaculty === faculty.id ? 'opacity-100' : 'opacity-0'}`} />
                  </div>
                  {faculty.name}
                </label>
              </div>
